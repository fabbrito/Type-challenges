...
type Join<Delimiter extends string, Parts extends string[]> = Parts extends [infer First, ...infer Others]
  ? `${First extends string ? First : ""}${Others extends [] ? "" : Delimiter}${Others extends string[]
      ? Join<Delimiter, Others>
      : ""}`
  : "";

declare function join<Delimiter extends string>(
  delimiter: Delimiter
): <Parts extends string[]>(...parts: Parts) => Join<Delimiter, Parts>;
