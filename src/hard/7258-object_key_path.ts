...
type NumberKey<K> = K extends number ? `[${K}]` | `.[${K}]` : never;
// Flag F: separates first run from the rest
type ObjectKeyPaths<T extends object, Flag extends boolean = false, K = keyof T> = K extends keyof T & (string | number)
  ?
      | (Flag extends true ? `.${K}` | NumberKey<K> : K)
      | (T[K] extends object ? `${Flag extends true ? `.${K}` | NumberKey<K> : K}${ObjectKeyPaths<T[K], true>}` : never)
  : never;
