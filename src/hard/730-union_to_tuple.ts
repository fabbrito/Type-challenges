...
// Proof https://www.typescriptlang.org/docs/handbook/release-notes/typescript-2-8.html#type-inference-in-conditional-types
// type P = "a" | "b";
// type P1 = ToIntersectionOfFunctions<P0>; // (() => "a") & (() => "b")
type ToIntersectionOfFunctions<T> = (T extends any ? (arg: () => T) => void : never) extends (arg: infer A) => void
  ? A
  : never;
  
// Infers the last R of the union T
//  type P2<T> = ToIntersectionOfFunctions<T> extends () => infer R ? R : never;
//  type P3 = P2<P>; // "b"
type UnionToTuple<T> = ToIntersectionOfFunctions<T> extends () => infer R ? [R, ...UnionToTuple<Exclude<T, R>>] : [];
