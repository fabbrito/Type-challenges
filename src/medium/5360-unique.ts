...
type Includes<U extends unknown[], V extends unknown> = U extends [infer UF, ...infer UR]
  ? (<T>() => T extends V ? 1 : 0) extends <T>() => T extends UF ? 1 : 0
    ? true
    : Includes<UR, V>
  : false;

type Unique<T extends unknown[], U extends unknown[] = []> = T extends [infer TF, ...infer TR]
